...
const db = app.firestore().collection("pushing");

const MainBuiltDetail = ({ team }) => {
  const { id } = useParams();
  const dispatch = useDispatch();
  const stateData = useSelector((state) => state.myReducer.taskState);
  console.log("this si eruah", id);

  const goingData = useSelector(
    (state) => state.myReducer.onGoState.projectTask
  );
  ...
  const onChangeButtonData = async () => {
    await app.firestore().collection("pushing").doc(id).update({
          ...
          <Card>
            ...
            <Placed>
              {stateData.map((propsData) => (
                ...
                >
                  {propsData.projectTask.map((props) => (
                    <div>
                      <div>{props.task}</div>
                      <button
                        onClick={() => {
                          ...
                          dispatch(addingGoing(propsData));
                          dispatch(removingItem(propsData));
                        }}
                        style={{
                          height: "40px",
                          background: "red",
                  ...
                  <TaskMembers team={propsData.projectTask[0].team} />
                </div>
              ))}
            </Placed>{" "}
          </Card>
